// clipster-settings — Qt Widgets editor for %APPDATA%/Clipster/settings.json.
// The tray app watches the file and hot-reloads it after Save.
...
class SettingsDialog : public QDialog {
 public:
  SettingsDialog(Settings settings, std::filesystem::path path)
      : settings_(std::move(settings)), path_(std::move(path)) {
    setWindowTitle(tr("Clipster Settings"));
 ...
 private:
  QWidget* build_recording_tab() {
    ...
    fps_->setCurrentIndex(std::max(0, fps_->findData(settings_.recording.fps)));
    form->addRow(tr("Frame rate:"), fps_);

    bitrate_ = new QSpinBox;
    bitrate_->setRange(1, 150);
    bitrate_->setSuffix(tr(" Mbps"));
    bitrate_->setValue(settings_.recording.bitrate_kbps / 1000);
    ...
    codec_->setCurrentIndex(
        std::max(0, codec_->findData(QString::fromStdString(settings_.recording.codec))));
    form->addRow(tr("Codec:"), codec_);

    buffer_ = new QSpinBox;
    buffer_->setRange(5, 600);
    buffer_->setSuffix(tr(" s"));
    buffer_->setValue(settings_.recording.buffer_seconds);
  ...
  QWidget* build_output_tab() {
    ...
    save_dir_ = new QLineEdit(QString::fromStdString(settings_.output.save_dir));
    ...
    subfolder_->setChecked(settings_.output.subfolder_per_game);
    form->addRow(QString(), subfolder_);

    template_ = new QLineEdit(QString::fromStdString(settings_.output.filename_template));
    ...
    clip_len_->setValue(settings_.clip.default_length_seconds);
  ...
  QWidget* build_audio_tab() {
    ...
    audio_mode_->setCurrentIndex(
        std::max(0, audio_mode_->findData(QString::fromStdString(settings_.audio.mode))));
    ...
    inc_layout->addWidget(make_list_editor(
        include_list_, to_qstringlist(settings_.audio.include_apps), tr("Add app…"),
    ...
    exc_layout->addWidget(make_list_editor(
        exclude_list_, to_qstringlist(settings_.audio.exclude_apps), tr("Add app…"),
  ...
  QWidget* build_games_tab() {
    ...
    steam_->setChecked(settings_.games.auto_detect_steam);
    ...
    fg->addWidget(make_list_editor(
        folders_list_, to_qstringlist(settings_.games.watched_folders), tr("Add folder…"),
        [](QWidget* p) {
    ...
    eg->addWidget(make_list_editor(
        exes_list_, to_qstringlist(settings_.games.manual_exes), tr("Add exe…"),
        [](QWidget* p) {
    ...
    ig->addWidget(make_list_editor(
        ignored_list_, to_qstringlist(settings_.games.ignored_exes), tr("Add name…"),
  ...
  QWidget* build_hotkeys_tab() {
    ...
    sound_file_ = new QLineEdit(QString::fromStdString(settings_.notifications.sound_file));
    sound_file_->setPlaceholderText(tr("(default chime)"));
    auto* browse = new QPushButton(tr("Browse…"));
    auto* test = new QPushButton(tr("Test"));
    connect(browse, &QPushButton::clicked, this, [this] {
      const QString wav = QFileDialog::getOpenFileName(this, tr("Clip sound"), {},
                                                       tr("Wave audio (*.wav)"));
      if (!wav.isEmpty()) {
  ...
  void save() {
    settings_.recording.fps = fps_->currentData().toInt();
    settings_.recording.bitrate_kbps = bitrate_->value() * 1000;
    settings_.recording.codec = codec_->currentData().toString().toStdString();
    settings_.recording.buffer_seconds = buffer_->value();

    settings_.output.save_dir = save_dir_->text().trimmed().toStdString();
    settings_.output.filename_template = template_->text().toStdString();
    settings_.output.subfolder_per_game = subfolder_->isChecked();
    settings_.clip.default_length_seconds = clip_len_->value();

    settings_.audio.mode = audio_mode_->currentData().toString().toStdString();
    settings_.audio.include_apps = from_list_widget(include_list_);
    settings_.audio.exclude_apps = from_list_widget(exclude_list_);

    settings_.games.auto_detect_steam = steam_->isChecked();
    settings_.games.watched_folders = from_list_widget(folders_list_);
...
int main(int argc, char** argv) {
  ...
  std::string warning;
  Settings settings = Settings::load_or_default(path, &warning);
  if (!warning.empty()) {
    QMessageBox::warning(nullptr, "Clipster", QString::fromStdString(warning));
  }

  SettingsDialog dialog(std::move(settings), path);
  dialog.exec();
  return 0;
}
